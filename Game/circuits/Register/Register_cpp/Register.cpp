...
void RangeProof_10_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void RangeProof_10_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 1984;}
...
uint csoffset = mySignalStart+1880;
uint aux_dimensions[1] = {2};
for (uint i = 0; i < 2; i++) {
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[2]); // line circom 26
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[2]].signalStart + 0],&circuitConstants[2]); // line circom 34
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0],&circuitConstants[2]); // line circom 39
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[5]].signalStart + 0],&circuitConstants[2]); // line circom 53
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[6]].signalStart + 0],&circuitConstants[2]); // line circom 58
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[2]); // line circom 65
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[7]].signalStart + 0],&circuitConstants[2]); // line circom 72
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[8]].signalStart + 0],&circuitConstants[2]); // line circom 77
...
RangeProof_10_create(1,0,ctx,"main",0);
RangeProof_10_run(0,ctx);
}
